...
int main(void) {
    ...
    assert(accupos_get_dialogue_num(accupos) == accupos->n_dialogues);
    for (int i = 0; i < accupos->n_dialogues; i++) {
        Accupos_Dialogue *dialogue = &accupos->dialogues[i];

        assert(dialogue->raw == accupos_get_ith_raw(accupos, i));
        assert(dialogue->pos_x == accupos_get_ith_pos_x(accupos, i));
        assert(dialogue->pos_y == accupos_get_ith_pos_y(accupos, i));
        assert(dialogue->width == accupos_get_ith_width(accupos, i));
        assert(dialogue->height == accupos_get_ith_height(accupos, i));
